...
export default function FieldEditorModal({ formInfo, onClose }) {
    ...
    const deleteField = useCallback(async (fieldId) => {
        ...
        if (result.isConfirmed) {
            try {
                ...
                Swal.fire({ icon: 'success', title: 'Đã xóa!', text: 'Trường đã được xóa thành công.', customClass: swalCustomClass });
                fetchFieldList();
            } catch (error) {
    ...
    // ... Các hàm drag-drop có thể thêm vào đây nếu cần ...
                            ...
                            {customFields.length > 0 ? (
                                customFields.map((field, index) => (
                                 ...
                                 <button
                                    ...
                                    className="flex items-center justify-center gap-2 bg-blue-100 hover:bg-blue-200 text-blue-800 px-4 py-2 mt-3 rounded-lg w-full font-semibold border border-dashed border-blue-400"
                                >
                                    <PlusIcon className="h-5 w-5" />
                                    Thêm trường để cấu hình
                                </button>
                            )}
                            ...
                            {customFields.length > 0 && (
                                <div className="mt-4 flex gap-2">
                                     <button
                                        ...
                                        className="flex items-center justify-center gap-2 bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg font-semibold shadow-md flex-grow"
                                        disabled={isSaving}
                                    >
                                        {isSaving ? 'Đang lưu...' : <><ArrowDownTrayIcon className="h-5 w-5" /> {isFieldFormInEditMode ? 'Cập nhật trường' : 'Lưu trường mới'}</>}
                                    </button>
                                     <button type="button" onClick={() => { setCustomFields([]); setIsFieldFormInEditMode(false); setFieldToEdit(null);}} className="p-2 bg-gray-200 rounded-lg"><XMarkIcon className="h-5 w-5"/></button>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="w-full lg:w-2/3 h-full overflow-y-auto border-l border-gray-200 pl-6 custom-scrollbar">
                             ...
                             {fieldsData.length > 0 ? (
                                 fieldsData.map((item, index) => (
                                     <div key={item.id} className="field_item bg-gray-100 border rounded-lg p-3 flex justify-between items-center mb-2 shadow-sm cursor-grab">
                                ...
                                <div className="text-center text-gray-500 p-8 mt-10 border-2 border-dashed rounded-lg">
                                    <p>Chưa có trường nào được tạo cho biểu mẫu này.</p>
                                    <p className="text-sm mt-1">Sử dụng panel bên trái để thêm các trường mới.</p>
                                </div>
                             )}
                        </div>
                    </div>
